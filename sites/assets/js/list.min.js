...
this.set_primary_action();if(me.no_toolbar||me.hide_toolbar){me.$w.find('.list-toolbar-wrapper').toggle(false);}},set_primary_action:function(){var me=this;if(this.new_doctype){if(this.listview.settings.set_primary_action){this.listview.settings.set_primary_action(this);}else{this.page.set_primary_action(__("New"),function(){me.make_new_doc(me.new_doctype);},"octicon octicon-plus");}}else{this.page.clear_primary_action();}},make_new_doc:function(doctype){var me=this;frappe.model.with_doctype(doctype,function(){if(me.custom_new_doc){me.custom_new_doc(doctype);}else{var doc=frappe.model.get_new_doc(doctype);if(me.filter_list){frappe.route_options={};$.each(me.filter_list.get_filters(),function(i,f){if(f[2]==="="&&!in_list(frappe.model.std_fields_list,f[1])){frappe.route_options[f[1]]=f[3];}});}
...
if(frappe.route_options){this.set_route_options();this.run();}else if(this.dirty){this.run();}else{if(new Date()-(this.last_updated_on||0)>30000){this.run();}}},set_route_options:function(){var me=this;me.filter_list.clear_filters();$.each(frappe.route_options,function(key,value){var doctype=me.doctype;if(key.indexOf(".")!==-1){doctype=key.split(".")[0];key=key.split(".")[1];}
if($.isArray(value)){me.filter_list.add_filter(doctype,key,value[0],value[1]);}else{me.filter_list.add_filter(doctype,key,"=",value);}});frappe.route_options=null;},run:function(more){var me=this;if(this.fresh&&!more){return;}
...
this.list_header.find(".list-liked-by-me").toggleClass("text-extra-muted not-liked",!this.is_star_filtered());this.last_updated_on=new Date();this.dirty=false;this.fresh=true;setTimeout(function(){me.fresh=false;},1000);this._super(more);if(this.listview.settings.post_render){this.listview.settings.post_render(this);}},make_no_result:function(){var new_button=frappe.boot.user.can_create.indexOf(this.doctype)!=-1?('<p><button class="btn btn-default btn-sm" \
...
this.make_bulk_assignment();this.make_bulk_printing();},make_bulk_assignment:function(){var me=this;me.page.add_menu_item(__("Assign To"),function(){docname=[];$.each(me.get_checked_items(),function(i,doc){docname.push(doc.name);})
...
if(!no_run)
this.run();},call_for_selected_items:function(method,args){var me=this;args.names=$.map(this.get_checked_items(),function(d){return d.name;});frappe.call({method:method,args:args,freeze:true,callback:function(r){if(!r.exc){me.list_header.find(".list-select-all").prop("checked",false);me.refresh();}}});}});;frappe.provide('frappe.views');frappe.views.ListSidebar=Class.extend({init:function(opts){$.extend(this,opts);this.make();this.get_stats();},make:function(){var sidebar_content=frappe.render_template("list_sidebar",{doctype:this.doclistview.doctype});this.offcanvas_list_sidebar=$(".offcanvas .list-sidebar").html(sidebar_content);this.page_sidebar=$('<div class="list-sidebar hidden-xs hidden-sm"></div>').html(sidebar_content).appendTo(this.page.sidebar.empty());this.sidebar=this.page_sidebar.add(this.offcanvas_list_sidebar);this.setup_assigned_to_me();if(frappe.views.calendar[this.doctype]){this.sidebar.find(".calendar-link, .gantt-link").removeClass("hide");}},setup_assigned_to_me:function(){var me=this;this.page.sidebar.find(".assigned-to-me a").on("click",function(){me.doclistview.assigned_to_me();});this.offcanvas_list_sidebar.find(".assigned-to-me a").on("click",function(){me.doclistview.assigned_to_me();});},get_stats:function(){var me=this
...
frappe.templates["list_item_row"] = '<div class="row doclist-row {% if (data._checkbox) { %} has-checkbox {% } %}"> <div class="col-xs-10 {% if (list.meta.title_field && !list.settings.hide_name_column) { %} col-sm-8 {% } else { %} col-sm-10 {% } %} list-row-left">  {%= main %} </div>  {% if (list.meta.title_field && !list.settings.hide_name_column) { var is_different = data.name !== data[list.meta.title_field]; %} <div class="list-col col-sm-2 hidden-xs text-right text-ellipsis list-row-id"> {% if (is_different) { %} <a class="text-muted list-value" href="#Form/{%= data._doctype_encoded %}/{%= data._name_encoded %}"> {%= data.name %}</a> {% } %} </div> {% } %}  <div class="list-col col-sm-2 col-xs-2 text-right list-row-right" style="padding-left:0px"> <div class="visible-xs list-row-indicator">{%= list.get_indicator_dot(data) %}</div> <div class="hidden-xs"> <span class="list-row-modified text-muted"> {%= comment_when(data.modified, true) %} </span> {% if (data._assign_list.length) { %} <span class="filterable" data-filter="_assign,like,%{%= data._assign_list[data._assign_list.length - 1] %}%"> {%= frappe.avatar(data._assign_list[data._assign_list.length - 1]) %}</span> {% } else { %} <span class="avatar avatar-small avatar-empty"></span> {% } %} <span class="list-comment-count small {% if(!data._comment_count) { %} text-extra-muted {% } else { %} text-muted {% } %}"> <i class="octicon octicon-comment-discussion"></i> {%= (data._comment_count > 99 ? "99+" : data._comment_count) || 0 %} </span> </div> </div> </div> ';
frappe.templates["list_item_main_head"] = '<div class="row"> {% var total_cols=0; for (var i=0, l=columns.length; i < l; i++ ) { var col = columns[i]; total_cols += parseInt(col.colspan); %} {% if (total_cols <= 12) { %} <div class="col-sm-{%= col.colspan %} list-col text-ellipsis h6 text-muted {% if(col.type==="Subject") { if (right_column) { %} col-xs-9 {% } else { %} col-xs-12 {% } %} {% } else if(right_column && col.fieldname===right_column) { %} col-xs-3 {% } else { %} hidden-xs {% } %} {% if(col.df && ["Int", "Float", "Currency", "Percent"].indexOf(col.df.fieldtype)!==-1) { %}text-right{% } %}"> {% if (col.type==="Subject") { %} {% if (_checkbox) { %} <input class="list-select-all" type="checkbox" style="margin-right: 7px; margin-top: 2px;" title="{%= __("Select All") %}"> {% } %} <i class="icon-fixed-width octicon octicon-heart text-extra-muted not-liked like-action list-liked-by-me" title="{%= __("Likes") %}"></i> {% } %} <span class="list-value">{%= __(col.title) || __(col.label) || "" %}</span> </div> {% } %} {% } %} </div> ';
...
$.each(['name','owner','docstatus','_user_tags','_comments','modified','modified_by','_assign','_liked_by'],function(i,fieldname){add_field(fieldname);})
...
if(v.start&&v.end){source.push({name:v.title,desc:v.status,values:[{name:v.title,desc:v.desc,from:'/Date('+moment(v.start).format("X")+'000)/',to:'/Date('+moment(v.end).format("X")+'000)/',customClass:{'danger':'ganttRed','warning':'ganttOrange','info':'ganttBlue','success':'ganttGreen','':'ganttGray'}[v.cssClass],dataObj:v}]})}});return source}});;