...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/jquery/jquery-ui.min.js
 */
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/Sortable.min.js
 */
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/tag-it.min.js
 */
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/bootstrap.min.js
 */
/*!
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/moment/moment-with-locales.min.js
 */
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/moment/moment-timezone-with-data.min.js
 */
//! moment-timezone.js
//! version : 0.3.1
//! author : Tim Wood
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/socket.io.min.js
 */
...
return value||"";},Datetime:function(value){return value?dateutil.str_to_user(dateutil.convert_to_user_tz(value)):"";},Text:function(value){if(value){var tags=["<p","<div","<br","<table"];var match=false;for(var i=0;i<tags.length;i++){if(value.match(tags[i])){match=true;break;}}
if(!match){value=replace_newlines(value);}}
return frappe.form.formatters.Data(value);},LikedBy:function(value){var html="";$.each(JSON.parse(value||"[]"),function(i,v){if(v)html+='<span class="avatar avatar-small" \
...
return frappe.languages;};frappe.setup_language_field=function(frm,fieldname){if(!fieldname)fieldname='language';frm.set_df_property(fieldname,"options",[''].concat(frappe.get_languages())||["","english"]);frm.get_field(fieldname).set_input(frm.doc[fieldname]||'');};frappe.templates["modal"] = '<div class="modal fade" style="overflow: auto;" tabindex="-1"> <div class="modal-dialog"> <div class="modal-content"> <div class="modal-header"> <div class="row"> <div class="col-xs-7"> <h4 class="modal-title">{%= title %}</h4> </div> <div class="col-xs-5"> <div class="text-right buttons"> <button type="button" class="btn btn-default btn-sm btn-modal-close" data-dismiss="modal"> <i class="octicon octicon-x visible-xs" style="padding: 1px 0px;"></i> <span class="hidden-xs">{%= __("Close") %}</span></button> <button type="button" class="btn btn-primary btn-sm hide"> {%= __("Confirm") %}</button> </div> </div> </div> </div> <div class="modal-body ui-front">{%= content %} </div> </div> </div> </div> ';
...
ele.setAttribute('id',id);frappe.dom.id_count++;return id;},eval:function(txt){if(!txt)return;var el=document.createElement('script');el.appendChild(document.createTextNode(txt));document.getElementsByTagName('head')[0].appendChild(el);},remove_script_and_style:function(txt){var div=document.createElement('div');div.innerHTML=txt;["script","style","noscript","title","meta","base","head"].forEach(function(e,i){var elements=div.getElementsByTagName(e);var i=elements.length;while(i--){elements[i].parentNode.removeChild(elements[i]);}});return div.innerHTML;},set_style:function(txt,id){if(!txt)return;var se=document.createElement('style');se.type="text/css";if(id){var element=document.getElementById(id);if(element){element.parentNode.removeChild(element);}
se.id=id;}
...
if(msg_dialog.msg_area.html())msg_dialog.msg_area.append("<hr>");msg_dialog.msg_area.append(msg);msg_dialog.loading_indicator.addClass("hide");msg_dialog.show_loading=function(){msg_dialog.loading_indicator.removeClass("hide");}
msg_dialog.$wrapper.css("z-index",2000);msg_dialog.show();return msg_dialog;}
frappe.hide_msgprint=function(instant){if(msg_dialog&&msg_dialog.$wrapper.is(":visible")){if(instant){msg_dialog.$wrapper.removeClass("fade");}
...
function show_alert(txt,seconds){if(!$('#dialog-container').length){$('<div id="dialog-container"><div id="alert-container"></div></div>').appendTo('body');}
...
frappe.socket.socket.on('msgprint',function(message){frappe.msgprint(message);});frappe.socket.setup_listeners();frappe.socket.setup_reconnect();$(document).on('form-load form-rename',function(e,frm){if(frm.is_new()){return;}
...
if(cur_frm.doc){frappe.socket.doc_close(cur_frm.doctype,cur_frm.docname);}}},get_host:function(){var host=window.location.origin;if(window.dev_server){parts=host.split(":");if(parts.length>2){host=parts[0]+":"+parts[1];}
host=host+":3000";}
...
if(data){var opts=frappe.socket.open_tasks[data.task_id];if(opts[method])opts[method](data);if(method==="success"){if(opts.callback)opts.callback(data);}}
...
frappe.provide("frappe.realtime");frappe.realtime.on=function(event,callback){if(frappe.socket.socket){frappe.socket.socket.on(event,callback);}}
...
d=defaults[frappe.model.scrub(key)];if($.isArray(d))d=d[0];return d;},get_user_defaults:function(key){var defaults=frappe.boot.user.defaults;var d=defaults[key];if(frappe.defaults.is_a_user_permission_key(key)){if(d&&$.isArray(d)&&d.length===1){d=d[0];}else{d=defaults[frappe.model.scrub(key)];}}
...
this.fields=frappe.meta.sort_docfields(frappe.meta.docfield_map[this.doctype]);this.setup_tabbing();this.render();},show_empty_form_message:function(){this.wrapper.find(".empty-form-alert").remove();if(!(this.wrapper.find(".frappe-control:visible").length)){$('<div class="empty-form-alert text-muted" style="padding: 15px;">'
+__("This form does not have any input")+'</div>').appendTo(this.page);}},render:function(){var me=this;this.section=null;this.column=null;if((this.fields[0]&&this.fields[0].fieldtype!="Section Break")||!this.fields.length){this.make_section();}
...
cnt++;}});},refresh_section_collapse:function(){if(!this.doc)return;for(var i=0;i<this.sections.length;i++){var section=this.sections[i];var df=section.df;if(df&&df.collapsible){if(df.collapsible_depends_on){var collapse=!this.evaluate_depends_on_value(df.collapsible_depends_on);var has_error=section.wrapper.find(".frappe-control.has-error").length;if(collapse&&has_error){collapse=false;}
section.collapse(collapse);}}}},attach_doc_and_docfields:function(refresh){var me=this;for(var i=0,l=this.fields_list.length;i<l;i++){var fieldobj=this.fields_list[i];if(me.doc){fieldobj.doc=me.doc;fieldobj.doctype=me.doc.doctype;fieldobj.docname=me.doc.name;fieldobj.df=frappe.meta.get_docfield(me.doc.doctype,fieldobj.df.fieldname,me.frm.doc.name);fieldobj.perm=me.frm.perm;};refresh&&fieldobj.refresh&&fieldobj.refresh();}},refresh_section_count:function(){this.wrapper.find(".section-count-label:visible").each(function(i){$(this).html(i+1);});},setup_tabbing:function(){var me=this;this.wrapper.on("keydown",function(ev){if(ev.which==9){var current=$(ev.target),doctype=current.attr("data-doctype"),fieldname=current.attr("data-fieldname");if(doctype)
return me.handle_tab(doctype,fieldname,ev.shiftKey);}})},handle_tab:function(doctype,fieldname,shift){var me=this,grid_row=null;prev=null,fields=me.fields_list,in_grid=false,focused=false;if(doctype!=me.doctype){grid_row=me.get_open_grid_row();fields=grid_row.layout.fields_list;}
...
this.refresh_section_count();},evaluate_depends_on_value:function(expression){var out=null;var doc=this.doc;if(!doc)return;var parent=this.frm?this.frm.doc:null;if(expression.substr(0,5)=='eval:'){out=eval(expression.substr(5));}else if(expression.substr(0,3)=='fn:'&&me.frm){out=this.frm.script_manager.trigger(expression.substr(3),this.doctype,this.docname);}else{var value=doc[expression];if($.isArray(value)){out=!!value.length;}else{out=!!value;}}
return out;}});frappe.ui.form.Section=Class.extend({init:function(layout,df){var me=this;this.layout=layout;this.df=df||{};this.make();this.row={wrapper:this.wrapper};if(this.df.collapsible){this.collapse(true);}
...
this.body=$('<div class="section-body">').appendTo(this.wrapper);},make_head:function(){var me=this;if(!this.df.collapsible){$('<div class="col-sm-12"><h4 class="form-section-heading">'
+__(this.df.label)+'</h4></div>').appendTo(this.wrapper);}else{this.head=$('<div class="section-head"><a class="h6">'
...
this.body.toggleClass("hide",hide);this.head.toggleClass("collapsed",hide);this.indicator.toggleClass("octicon-chevron-down",hide);this.indicator.toggleClass("octicon-chevron-up",!hide);}})
...
return false;});$('<div class="clearfix"></div>').appendTo(this.$wrapper);}});frappe.ui.form.ControlInput=frappe.ui.form.Control.extend({horizontal:true,make:function(){this._super();this.set_input_areas();this.set_max_width();this.setup_update_on_refresh();},make_wrapper:function(){if(this.only_input){this.$wrapper=$('<div class="form-group frappe-control">').appendTo(this.parent);}else{this.$wrapper=$('<div class="frappe-control">\
   ...
   </div>').appendTo(this.parent);}},toggle_label:function(show){this.$wrapper.find(".control-label").toggleClass("hide",!show);},toggle_description:function(show){this.$wrapper.find(".help-box").toggleClass("hide",!show);},set_input_areas:function(){if(this.only_input){this.input_area=this.wrapper;}else{this.label_area=this.label_span=this.$wrapper.find("label").get(0);this.input_area=this.$wrapper.find(".control-input").get(0);this.disp_area=this.$wrapper.find(".control-value").get(0);}},set_max_width:function(){if(this.horizontal){this.$wrapper.addClass("input-max-width");}},setup_update_on_refresh:function(){var me=this;this.$wrapper.on("refresh",function(){if(me.disp_status!="None"){if(me.doctype&&me.docname){me.value=frappe.model.get_value(me.doctype,me.docname,me.df.fieldname);}
...
return[];return frappe.utils.filter_dict(docsdict,filters);},get_value:function(doctype,filters,fieldname,callback){if(callback){frappe.call({method:"frappe.client.get_value",args:{doctype:doctype,fieldname:fieldname,filters:filters},callback:function(r){if(!r.exc){callback(r.message);}}});}else{if(typeof filters==="string"&&locals[doctype]&&locals[doctype][filters]){return locals[doctype][filters][fieldname];}else{var l=frappe.get_list(doctype,filters);return(l.length&&l[0])?l[0][fieldname]:null;}}},set_value:function(doctype,docname,fieldname,value,fieldtype){var doc=locals[doctype]&&locals[doctype][docname];if(doc&&doc[fieldname]!==value){doc[fieldname]=value;frappe.model.trigger(fieldname,value,doc);return true;}else{if(fieldtype=="Link"&&doc){frappe.model.trigger(fieldname,value,doc);}}},on:function(doctype,fieldname,fn){frappe.provide("frappe.model.events."+doctype);if(!frappe.model.events[doctype][fieldname]){frappe.model.events[doctype][fieldname]=[];}
...
frappe.model.add_to_locals(doc);if(!parent_doc){doc.__run_link_triggers=1;}
return doc;},make_new_doc_and_get_name:function(doctype){return frappe.model.get_new_doc(doctype).name;},get_new_name:function(doctype){var cnt=frappe.model.new_name_count
if(!cnt[doctype])
...
var user_default=frappe.defaults.get_user_default(df.fieldname);var is_allowed_user_default=user_default&&(!has_user_permissions||user_permissions[df.options].indexOf(user_default)!==-1);if(is_allowed_user_default){return user_default;}}
...
frappe.set_route("Form",r.message.doctype,r.message.name);}}})},map_current_doc:function(opts){if(opts.get_query_filters){opts.get_query=function(){return{filters:opts.get_query_filters};}}
var _map=function(){return frappe.call({type:"POST",method:opts.method,args:{"source_name":opts.source_name,"target_doc":cur_frm.doc},callback:function(r){if(!r.exc){var doc=frappe.model.sync(r.message);cur_frm.refresh();}}});}
...
if(docinfo.shared){for(var i=0;i<docinfo.shared.length;i++){var s=docinfo.shared[i];if(s.user===user){perm[0]["read"]=perm[0]["read"]||s.read;perm[0]["write"]=perm[0]["write"]||s.write;perm[0]["share"]=perm[0]["share"]||s.share;if(s.read){perm[0].email=meta.permissions[0].email;perm[0].print=meta.permissions[0].print;}}}}}
...
/*
 *	/Users/gorjuce/projects/frappe-demo/apps/frappe/frappe/public/js/lib/md5.min.js
 */
...
return true;}},get_desktop_items:function(global){var modules_list=[];if(!global){var user_list=frappe.defaults.get_default("_desktop_items");if(user_list&&user_list.length)
var modules_list=user_list;}
frappe.boot.module_list.forEach(function(i,m){if(modules_list.indexOf(m)===-1){modules_list.push(m);}});if(modules_list){$.each(frappe.modules,function(m,module){var module=frappe.get_module(m);if(modules_list.indexOf(m)===-1){modules_list.push(m);}});}
...
return frappe.user.modules;},is_module_blocked:function(m){return frappe.boot.user.block_modules&&frappe.boot.user.block_modules.indexOf(m)!==-1;},is_report_manager:function(){return frappe.user.has_role(['Administrator','System Manager','Report Manager']);},get_formatted_email:function(email){var fullname=frappe.user.full_name(email);if(!fullname){return email;}else{var quote='';if(fullname.search(/[\[\]\\()<>@,:;".]/)!==-1){quote='"';}
...
document.body.appendChild(a);a.click();document.body.removeChild(a);};frappe.markdown=function(txt){if(!frappe.md2html){frappe.require('assets/frappe/js/lib/markdown.js');frappe.md2html=new Showdown.converter();}
...
$("body").on("click","a.help-link",function(){var doctype=$(this).attr("data-doctype");doctype&&frappe.help.show(doctype);});;frappe.templates["upload"] = '<div class="file-upload"> <div class="input-upload"> <input class="input-upload-file hidden" type="file" name="filedata" /> <button class="btn btn-primary btn-sm btn-browse">{%= __("Browse") %}</button> </div> <div class="uploaded-filename hidden" style="width: calc(100% - 67px);"></div> <div class="web-link-wrapper" style="width: calc(100% - 67px);"> <span class="text-muted file-upload-or">{%= __("or") %}</span> <div class="input-link" style="width: calc(100% - 30px);"> <div class="input-group"> <div class="input-group-addon"> <span class="hidden-xs">{%= __("Web Link") %}</span> <i class="icon-link visible-xs"></i> </div> <input class="form-control" type="text" name="file_url" placeholder="{%= (opts.sample_url || "e.g. http://example.com/somefile.png") %}"/> </div> </div> </div> <div class="private-file hidden"> <div class="checkbox"> <label> <input type="checkbox" checked> {{ __("Private") }} </label> </div> </div> </div> ';
...
ajax_args={"method":"uploadfile",args:args,callback:function(r){if(!r._server_messages)
msgbox.hide();if(r.exc){opts.onerror?opts.onerror(r):opts.callback(null,null,r);return;}
var attachment=r.message;opts.callback(attachment,r);$(document).trigger("upload_complete",attachment);},error:function(r){opts.onerror?opts.onerror(r):opts.callback(null,null,r);return;}}
...
$(document).trigger("page-change");this.page._route=window.location.hash;$(this.page).trigger('show');scroll(0,0);frappe.breadcrumbs.update();return this.page;},});;frappe.breadcrumbs={all:{},preferred:{"File":""},set_doctype_module:function(doctype,module){localStorage["preferred_breadcrumbs:"+doctype]=module;},get_doctype_module:function(doctype){return localStorage["preferred_breadcrumbs:"+doctype];},add:function(module,doctype,type){frappe.breadcrumbs.all[frappe.get_route_str()]={module:module,doctype:doctype,type:type};frappe.breadcrumbs.update();},update:function(){var breadcrumbs=frappe.breadcrumbs.all[frappe.get_route_str()];var $breadcrumbs=$("#navbar-breadcrumbs").empty();if(!breadcrumbs){$("body").addClass("no-breadcrumbs");return;}
...
if(breadcrumbs.doctype&&frappe.get_route()[0]==="Form"){if(breadcrumbs.doctype==="User"&&frappe.user.modules.indexOf("Setup")===-1){}else{route=(cur_frm&&cur_frm.list_route)||("List/"+breadcrumbs.doctype)
...
frappe.templates["offcanvas_left_sidebar"] = '<div class="main-menu"> <form role="search" onsubmit="return false;"> <div class="form-group form-group-sm"> <input id="sidebar-search" type="text" class="form-control" placeholder="{%= __("Search or type a command") %}" aria-haspopup="true"> <span class="octicon octicon-search navbar-search-icon"></span> </div> </form> <ul class="list-unstyled sidebar-menu"> <li class="divider"></li> <li><a href="#" class="strong">{%= __("Home") %}</a></li> </ul> <div class="form-sidebar" style="display: none"> </div> <div class="list-sidebar" style="display: none"> </div> <div class="module-sidebar" style="display: none"> </div> <ul class="desk-sidebar list-unstyled sidebar-menu" style="display: none"> <li class="divider"></li> <li><a href="#" onclick="return frappe.ui.toolbar.show_about();"> {%= __("About") %}</a></li> <li><a href="https://frappe.io" target="_blank" data-link="docs"> {%= __("Documentation") %}</a></li> <li><a href="https://discuss.erpnext.com" target="_blank"> {%= __("Forums") %}</a></li> <li><a href="https://github.com/frappe/erpnext/issues" target="_blank"> {%= __("Report an Issue") %}</a></li> <li><a href="#" onclick="return frappe.ui.toolbar.clear_cache();"> {%= __("Reload") %}</a></li> <li><a href="/index" target="_blank"> {%= __("View Website") %}</a></li> </ul> </div> <div class="user-menu clearfix"> <div class="pull-left text-ellipsis" style="max-width: 75%;"> <a href="#Form/User/{%= encodeURIComponent(user) %}"> <img src="{%= frappe.user_info().image %}" class="navbar-user-image" alt="{%= frappe.user.abbr() %}"> <span>{%= frappe.user.full_name() %}</span> </a> </div> <div class="pull-right" style="margin-top: 4px;"> <a onclick="return frappe.app.logout();"><i class="octicon octicon-sign-out"></i></a> </div> </div> ';
frappe.provide("frappe.ui.toolbar");frappe.ui.toolbar.Toolbar=Class.extend({init:function(){var header=$('header').append(frappe.render_template("navbar",{}));var sidebar=$('.offcanvas .sidebar-left').append(frappe.render_template("offcanvas_left_sidebar",{}));header.find(".toggle-sidebar").on("click",frappe.ui.toolbar.toggle_left_sidebar);header.find(".toggle-navbar-new-comments").on("click",function(){$(".offcanvas").toggleClass("active-right").removeClass("active-left");return false;});$(document).on("notification-update",function(){frappe.ui.notifications.update_notifications();});$('.dropdown-toggle').dropdown();$(document).trigger('toolbar_setup');$(document).on("page-change",function(){$("header .navbar .custom-menu").remove();});frappe.search.setup();},});$.extend(frappe.ui.toolbar,{add_dropdown_button:function(parent,label,click,icon){var menu=frappe.ui.toolbar.get_menu(parent);if(menu.find("li:not(.custom-menu)").length&&!menu.find(".divider").length){frappe.ui.toolbar.add_menu_divider(menu);}
return $('<li class="custom-menu"><a><i class="icon-fixed-width '
+icon+'"></i> '+label+'</a></li>').insertBefore(menu.find(".divider")).find("a").click(function(){click.apply(this);});},get_menu:function(label){return $("#navbar-"+label.toLowerCase());},add_menu_divider:function(menu){menu=typeof menu=="string"?frappe.ui.toolbar.get_menu(menu):menu;$('<li class="divider custom-menu"></li>').prependTo(menu);},toggle_left_sidebar:function(){$(".offcanvas").toggleClass("active-left").removeClass("active-right");return false;}});frappe.ui.toolbar.clear_cache=function(){frappe.assets.clear_local_storage();$c('frappe.sessions.clear',{},function(r,rt){if(!r.exc){show_alert(r.message);location.reload(true);}});return false;}
...
var count=notifications_map[doctype];if(count){var config=frappe.ui.notifications.config[doctype]||{};var label=config.label||doctype;var notification_row=repl('<li><a class="badge-hover" data-doctype="%(data_doctype)s">\
   <span class="badge pull-right">\
    %(count)s</span> \
   %(label)s </a></li>',{label:__(label),icon:frappe.boot.doctype_icons[doctype],count:count>99?'99+':count,data_doctype:doctype});frappe.ui.notifications.navbar_notification.append($(notification_row));frappe.ui.notifications.sidebar_notification.append($(notification_row));frappe.ui.notifications.total+=count;}}
frappe.ui.notifications.config={"ToDo":{label:__("To Do")},"Messages":{label:__("Messages"),route:"messages"},"Event":{label:__("Calendar"),route:"Calendar/Event"},"Likes":{label:__("Likes"),click:function(){frappe.route_options={show_likes:true};if(frappe.get_route()[0]=="activity"){frappe.pages['activity'].page.list.refresh();}else{frappe.set_route("activity");}}},};frappe.views.show_open_count_list=function(element){var doctype=$(element).attr("data-doctype");var condition=frappe.boot.notification_info.conditions[doctype];if(condition&&$.isPlainObject(condition)){frappe.route_options=condition;}
...
return module;};;function get_url_arg(name){return get_query_params()[name]||"";}
...
return query_params;}
function make_query_string(obj){var query_params=[];$.each(obj,function(k,v){query_params.push(encodeURIComponent(k)+"="+encodeURIComponent(v));});return"?"+query_params.join("&");};frappe.templates["error_object"] = '{% if (Object.prototype.toString.call(x) === "[object Object]") { %} <table class="table"> {% for (var key in x) { %} <tr> <td><code>{{ key }}</code></td> <td>{{ x[key] }}</td> </tr> {% } %} </table> {% } else { %} {{ x }} {% } %} ';
frappe.templates["error_snapshot"] = '<style> a { cursor: pointer; } .codebox { font-family: monospace; font-size: 8pt; } .codebox .line.current { background: rgba(0,0,255, 0.1); } .codebox .lineno { text-align: right; display: inline-block; width: 30px; opacity: .5; } .codebox .code { white-space: pre; } .object-link { font-family: monospace; white-space: pre; } </style> {% function id(){ return id._old_id++; }; id._old_id = 0; %} <h3>{{ __("Error Report") }}</h3> <p class="text-muted">{{ doc.pyver }}</p> <dl> <dt>{{ __("Timestamp") }}: </dt> <dd>{{ doc.timestamp }}</dd> <dt>{{ __("Relapsed") }}</dt> <dd><code>{{ doc.relapses }}</code></dd> </dl> <h3>{{ __("Exception") }}</h3> {{ frappe.render_template("error_object", {x: JSON.parse(doc.exception)}) }} <h3>{{ __("Locals") }}</h3> {{ frappe.render_template("error_object", {x: JSON.parse(doc.locals)} )}} <h3>{{ __("Traceback") }}</h3> {% var frames = JSON.parse(doc.frames); %} {% for (var i in frames) { %} {% var frameid = id(), frame = frames[i] %} <p><i class="octicon octicon-file-text"></i> <code>{{ frame.file }}: {{ frame.lnum }}</code> <div class="row"> <div class="codebox"> <div class="col-lg-11"> {% for (var index in frame.lines) { %} {% var line = frame.lines[index] %} <div class="line {{ index == frame.lnum ? "current": "" }}"> <span class="lineno text-muted">{{ index }}</span> <span class="code">{{ line }}</span> </div> {% } %} </div> <div class="col-lg-1"> <span class="btn btn-xs btn-default" data-toggle="collapse" data-target="#frame-{{ frameid }}-locals"> <i class="icon-list-ul"> {{ __("Locals") }}</i> </span> </div> </div> </div> <div class="row"> <div class="col-lg-12 collapse" id="frame-{{ frameid }}-locals"> <h4>{{ __("Locals") }}</h4> {{ frappe.render_template("error_object", {x: frame.dump }) }} </div> </div> </p> {% } %} ';
