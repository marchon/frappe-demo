...
this.sidebar=new frappe.ui.form.Sidebar({frm:this,page:this.page});this.refresh_header(is_a_different_doc);this.script_manager.trigger("refresh");$(document).trigger('form_refresh',[this]);this.refresh_fields();if(this.cscript.is_onload){this.script_manager.trigger("onload_post_render");}
...
$(cur_frm.wrapper).trigger('render_complete');this.layout.show_empty_form_message();this.scroll_to_element();}
_f.Frm.prototype.refresh_field=function(fname){if(cur_frm.fields_dict[fname]&&cur_frm.fields_dict[fname].refresh){cur_frm.fields_dict[fname].refresh();this.layout.refresh_dependency();}}
...
if(me.meta.autoname&&me.meta.autoname.substr(0,6)=='field:'&&!me.doc.__islocal){var fn=me.meta.autoname.substr(6);if(cur_frm.doc[fn]){cur_frm.toggle_display(fn,false);}}
...
frappe.after_ajax(function(){me.trigger_link_fields();});frappe.breadcrumbs.add(me.meta.module,me.doctype)})}
...
scroll(0,0);var after_save=function(r){if(!r.exc){if(["Save","Update","Amend"].indexOf(save_action)!==-1){frappe.utils.play_sound("click");}
me.script_manager.trigger("after_save");me.refresh();}else{if(on_error)
on_error();}
callback&&callback(r);if(frappe._from_link){if(me.doctype===frappe._from_link.df.options){frappe.model.set_value(frappe._from_link.frm.doctype,frappe._from_link.frm.docname,frappe._from_link.df.fieldname,me.docname);frappe._from_link.refresh();frappe.set_route("Form",frappe._from_link.frm.doctype,frappe._from_link.frm.docname);setTimeout(function(){scroll(0,frappe._from_link_scrollY);},100);}
...
_f.Frm.prototype.get_field=function(field){return cur_frm.fields_dict[field];};_f.Frm.prototype.new_doc=function(doctype,field,opts){frappe._from_link=field;frappe._from_link_scrollY=$(document).scrollTop();new_doc(doctype,opts);}
...
this.refresh(true);},toggle_reqd:function(fieldname,reqd){this.get_docfield(fieldname).reqd=reqd;this.refresh();},toggle_enable:function(fieldname,enable){this.get_docfield(fieldname).read_only=enable?0:1;;this.refresh();},get_docfield:function(fieldname){return frappe.meta.get_docfield(this.doctype,fieldname,this.frm?this.frm.docname:null);},get_field:function(fieldname){if(!this.fieldinfo[fieldname])
...
frappe.provide("frappe.ui.form");frappe.ui.form.Viewers=Class.extend({init:function(opts){$.extend(this,opts);},refresh:function(data_updated){var me=this;this.parent.empty();var viewers=this.frm.get_docinfo().viewers||{};var users=[];var new_users=[];for(var i=0,l=(viewers.current||[]).length;i<l;i++){var username=viewers.current[i];if(username===user){continue;}
var user_info=frappe.user_info(username);users.push({image:user_info.image,fullname:user_info.fullname,title:__("{0} is currently viewing this document",[user_info.fullname])});if(viewers.new.indexOf(username)!==-1){new_users.push(user_info.fullname);}}
...
frappe.templates["timeline_item"] = '<div class="media timeline-item {% if (data.user_content) { %} user-content {% } else { %} notification-content {% } %}" data-doctype="{{ data.doctype }}" data-name="{%= data.name %}"> {% if (data.user_content) { %} <span class="pull-left avatar avatar-medium"> <div class="avatar-frame" style="background-image: url({%= data.image %})"></div> </span> {% } %} <div class="pull-left media-body" style="max-width: calc(100% - 50px); padding-right: 0px;"> <div class="media-content-wrapper"> <div class="pull-right close-btn-container"> <span class="small text-muted"> {%= data.delete %} </span> </div> {% if(data.communication_type==="Communication" || (data.communication_type==="Comment" && data.comment_type==="Comment")) { %} <div class="comment-header small"> <i class="{%= data.icon %} icon-fixed-width"></i> <span title="{%= data.comment_by %}">{%= data.fullname %}</span> <span> {% if (data.timeline_doctype===data.frm.doc.doctype && data.timeline_name===data.frm.doc.name) { %} &ndash; <a href="#Form/{%= data.reference_doctype %}/{%= data.reference_name %}" class="text-muted"> <strong>{{ __(data.reference_doctype) }}</strong> {{ data.reference_name }} </a> {% } %} </span> <span class="text-muted" style="font-weight: normal;"> &ndash; {%= data.comment_on %}</span> {% if(data.communication_type==="Communication") { %} {% if (frappe.model.can_read(\'Communication\')) { %} <a href="#Form/{%= data.doctype %}/{%= data.name %}" class="text-muted"> {% } %} {% if (data.delivery_status) { if (in_list(["Sent", "Opened", "Clicked"], data.delivery_status)) { var indicator_class = "green"; } else { var indicator_class = "red"; } %} <span class="text-muted">&ndash;</span> <span class="indicator-right {%= indicator_class %} delivery-status-indicator" title="{%= data.delivery_status %}"> {%= data.delivery_status %}</span> {% } else { %} {% if (frappe.model.can_read(\'Communication\')) { %} <span class="text-muted">&ndash;</span> {%= __("Details") %} {% } %} {% } %} {% if (frappe.model.can_read(\'Communication\')) { %} </a> {% } %} {% if (data.communication_medium === "Email") { %} <a class="text-muted reply-link pull-right" data-name="{%= data.name %}">{%= __("Reply") %}</a> {% } %} {% } %} <span class="comment-likes" data-liked-by=\'{{ JSON.stringify(data._liked_by) }}\'> <i class="octicon octicon-heart like-action {% if (!data.liked_by_user) { %} text-extra-muted not-liked {% } %} " data-doctype="{%= data.doctype %}" data-name="{%= data.name %}"></i> <span class="likes-count text-muted">{{ (data._liked_by || []).length }}</span> </span> </div> <div class="reply"> <div> {%= data.content_html %} </div> </div> {% } else if(in_list(["Assignment Completed", "Assigned", "Shared", "Unshared"], data.comment_type)) { %} <div class="small"> <i class="{%= data.icon %} icon-fixed-width"></i> {% if (data.timeline_doctype===data.frm.doc.doctype && data.timeline_name===data.frm.doc.name) { %} <a href="#Form/{%= data.reference_doctype %}/{%= data.reference_name %}"> <strong>{{ __(data.reference_doctype) }}</strong> {{ data.reference_name }} </a> &ndash; {% } %} {% if(data.link_doctype && data.link_name) { %} <a href="#Form/{%= data.link_doctype %}/{%= data.link_name %}"> {% } %} {%= data.content %} {% if(data.link_doctype && data.link_name) { %} </a> {% } %} <span class="text-muted" style="font-weight: normal;"> &ndash; {%= data.comment_on %}</span> </div> {% } else { %} <div class="small"> <i class="{%= data.icon %} icon-fixed-width"></i> {% if (data.comment_type == "Like") { %} <span title="{%= data.comment_by %}"> {% if (data.timeline_doctype===data.frm.doc.doctype && data.timeline_name===data.frm.doc.name) { %} <a href="#Form/{%= data.reference_doctype %}/{%= data.reference_name %}"> <strong>{{ __(data.reference_doctype) }}</strong> {{ data.reference_name }} </a> &ndash; {% } %} {%= __("Liked by {0}", [data.fullname]) %} </span> {% } else { %} <span title="{%= data.comment_by %}">{%= data.fullname %}</span> {%= data.content %} {% if (data.timeline_doctype===data.frm.doc.doctype && data.timeline_name===data.frm.doc.name) { %} &ndash; <a href="#Form/{%= data.reference_doctype %}/{%= data.reference_name %}"> <strong>{{ __(data.reference_doctype) }}</strong> {{ data.reference_name }} </a> {% } %} {% } %} <span class="text-muted" style="font-weight: normal;"> &ndash; {%= data.comment_on %}</span> </div> {% } %} {% if(data.attachments && data.attachments.length) { %} <div style="margin: 10px 0px"> {% $.each(data.attachments, function(i, a) { %} <div class="text-ellipsis"> <a href="{%= a.file_url.replace(/#/g, \'%23\') %}" class="text-muted small" target="_blank"> <i class="icon-paperclip"></i> {%= a.file_url.split("/").slice(-1)[0] %} {% if (a.is_private) { %} <i class="icon icon-lock text-warning"></i> {% } %} </a> </div> {% }); %} </div> {% } %} </div> </div> </div> ';
...
if(!c.sender)c.sender=this.frm.doc.owner;if(c.sender.indexOf("<")!==-1){c.sender=c.sender.split("<")[1].split(">")[0];}
c.image=frappe.user_info(c.sender).image||frappe.get_gravatar(c.sender);c.comment_on=comment_when(c.creation);c.fullname=c.sender_full_name||frappe.user.full_name(c.sender);if(c.attachments&&typeof c.attachments==="string")
